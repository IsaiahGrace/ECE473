...
(define (boolean-rewrite phi)
 ...
 (cond ((null? phi) '())
	...
	(if (list? (first phi))
       ...
       ((> (length phi) 2) (cons (boolean-rewrite (first phi)) (boolean-rewrite (rest phi))))
       (else (list (boolean-rewrite (first phi)) (boolean-rewrite (rest phi))))))
...
(define (cond-and phi)
 (if (= (length (rest phi)) 1)
     ...
     (append (list (first phi))
	     (let ((simple-phi  (boolean-rewrite (set-create (expand-and (rest phi))))))
...
(define (cond-or phi)
 (if (= (length (rest phi)) 1)
     (boolean-rewrite (second phi))
     (append (list (first phi))
	     (let ((simple-phi (boolean-rewrite (set-create (expand-or (rest phi))))))
	      (if (list? simple-phi) simple-phi (list simple-phi))))))

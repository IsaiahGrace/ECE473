...
;;;                                       (and) -> #t                                             (working)
;;;                                     (and Φ) -> Φ                                              (working)
;;;                  (and Φ1...Φm #t Φm+1...Φn) -> (andΦ1...Φm Φm+1...Φn)                         (working)
;;;                  (and Φ1...Φm #f Φm+1...Φn) -> #f                                             (working)
;;;     (and Φ1...Φl (and Φl+1...Φm) Φm+1...Φn) -> (andΦ1...Φl Φl+1...Φm Φm+1...Φn)               (working)
;;;       (and Φ1...Φl Φ Φl+1...Φm Φ Φm+1...Φn) -> (and Φ1...Φl Φ Φl+1...Φm Φm+1...Φn)            (working)
;;; (and Φ1...Φl (not Φ) Φl+1...Φm Φ Φm+1...Φn) -> #f                                             (working)
;;; (and Φ1...Φl Φ Φl+1...Φm (not Φ) Φm+1...Φn) -> #f                                             (working)
;;;                                        (or) -> #f                                             (working)
;;;                                      (or Φ) -> Φ                                              (working)
;;;                   (or Φ1...Φm #f Φm+1...Φn) -> (or Φ1...Φm Φm+1...Φn)                         (working)
;;;                   (or Φ1...Φm #t Φm+1...Φn) -> #t                                             (working)
;;;       (or Φ1...Φl (or Φl+1...Φm) Φm+1...Φn) -> (or Φ1...Φl Φl+1...Φm Φm+1...Φn)               (working)
;;;        (or Φ1...Φl Φ Φl+1...Φm Φ Φm+1...Φn) -> (or Φ1...Φl Φ Φl+1...Φm Φm+1...Φn)             (working)
;;;  (or Φ1...Φl (not Φ) Φl+1...Φm Φ Φm+1...Φn) -> #t                                             (working)
;;;   (or Φ1...Φl Φ Φl+1...Φm (notΦ) Φm+1...Φn) -> #t                                             (working)
...
(define (boolean-simplify phi)
 ...
 (let ((simple-phi (boolean-rewrite phi)))
  (cond ((null? phi) phi)
	((symbol? simple-phi) simple-phi)
	((boolean? simple-phi) simple-phi)
	((equal? phi simple-phi) phi)
	(else (boolean-simplify simple-phi)))))

(define (boolean-rewrite phi)
 ;;(display "boolean-rewrite, phi= ") (write phi) (newline)
 (cond ((null? phi) '())
       ((symbol? phi) phi)
       ...
       ((= (length phi) 1)
	(if (list? (first phi))
	    (let ((simple-phi (boolean-rewrite (first phi))))
	     (cond ((boolean? simple-phi) simple-phi)
		   ((list? simple-phi)
		    (if (or (eq? (first simple-phi) 'AND)
			    (eq? (first simple-phi) 'OR)
			    (eq? (first simple-phi) 'NOT))
			simple-phi
			(first simple-phi)))
		   (else simple-phi)))
	    (boolean-rewrite (first phi))))
       ((eq? (first phi) 'AND) (cond-and phi))
       ((eq? (first phi) 'OR) (cond-or phi))
       ((eq? (first phi) 'NOT) (cond-not phi))
       ((> (length phi) 2) (cons (boolean-rewrite (first phi))
				 (boolean-rewrite (rest phi))))
       (else (list (boolean-rewrite (first phi))
		   (boolean-rewrite (rest phi))))))
...
(define (**test-vector**)
 (list
  ...
  '(NOT (NOT (AND P4 P4 P3 (OR P3 P2 (OR P1 (NOT P1))))))
  '(NOT (NOT (OR P4 P3 (NOT (NOT #F)) (AND P5 #T P5))))
  '(OR P1 P2 P3 (AND P4 P5 P2 P4) P3 P1 (OR P3 P4) (AND P2 (AND P3 (NOT P4))))))

;; FAILING

(define (test-boolean-simplify)
 (let ((phi '(NOT (NOT (OR P4 P3 (NOT (NOT #F)) (AND P5 #T P5))))))
  (print-truth-table (truth-table phi))
  (boolean-simplify phi)))

(define (try-test-vector)
 (map (lambda (phi)
       (write phi) (newline))
      (map boolean-simplify (**test-vector**))))
...
(define (expand-and phi)
 ...
 (cond ((null? phi) '())
       ((symbol? phi) phi)
       ((eq? (first phi) '#t) (expand-and (rest phi)))
...
(define (expand-or phi)
 ...
 (cond ((null? phi) '())
       ((symbol? phi) phi)
       ((eq? (first phi) '#f) (expand-or (rest phi)))
...
(define (test-expand-or)
 (expand-or '(P1 P2 P3 (OR P4 P5 P2 #t P4) P3 P1 (AND P3 P4) (NOT P1) (OR P2 (OR P3 P4)))))
...
(define (cond-and phi)
 (if (= (length (rest phi)) 1)
     (boolean-rewrite (second phi))
     (append (list (first phi))
	     (let ((simple-phi
		    (boolean-rewrite (set-create (expand-and (rest phi))))))
	      (if (list? simple-phi) simple-phi (list simple-phi))))))
...
(define (cond-or phi)
 (if (= (length (rest phi)) 1)
     ...
     (append (list (first phi))
	     (let ((simple-phi
		    (boolean-rewrite (set-create (expand-or (rest phi))))))
	      (if (list? simple-phi) simple-phi (list simple-phi))))))

(define (cond-not phi)
 (if (and (list? (second phi)) (eq? (first (second phi)) 'NOT))
     (boolean-rewrite (second (second phi)))
     (append (list (first phi))
	     (list (boolean-rewrite (rest phi))))))
;;(let ((simple-phi (boolean-rewrite (rest phi))))
;;(if (list? simple-phi) simple-phi (list simple-phi))))))
...
(define (truth-tables-equivalent? table table-simple)
 (cond ((null? table-simple) #t)
       ((row-equivelence? (first table-simple)
			  (get-matching-rows table (first table-simple) '()))
...
(define (get-matching-rows table row matching-rows)
 ...
 (cond ((null? table) matching-rows)
	...
	(get-matching-rows (rest table)
			   row
			   (append matching-rows (list (first table)))))
...
(define (truth-table-contradiction? table)
 ...
 (cond ((null? table) #t)
       ((eq? (second (first table)) #f)
	(truth-table-contradiction? (rest table)))
       (else #f)))

;;; Helper function that prints out a nicely formatted truth table
(define (print-truth-table truth-table)
 (cond ((null? truth-table) '())
       (else (write (first truth-table))
	     (newline)
	     (print-truth-table (rest truth-table)))))
